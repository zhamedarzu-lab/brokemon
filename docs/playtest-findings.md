...
| # | Item | What was found |
|---|------|----------------|
| 73 | The bot went shopping on the way to work | Instrumented over five full runs: **not one strike** came from the door-requirement path, and **95–101** came from lateness, of which **78% were exactly 105 minutes** late and 92% were 90–105. A constant is a collision, not bad luck — `playDay` woke at 7, crossed town to the food bank, ate and drank, and only then started walking to a 9 o'clock shift with an hour of grace. Three strikes is a firing, so the rig was sacked every fifth day for an entire run and reported a game that fires you. **The fix was already written down in the other routine**: `brokedaleDay` puts the shift first and its comment describes this precise failure — "queuing at the night market first cost forty-five minutes and made the bot chronically late: it worked every day, was written up every day, and was fired and demoted back to picker every fifth day". It was found there, fixed there, and never carried across to the day that runs 400 times a seed |
...
| 79 | The bot had no way to treat a fever in Brokemon | Every collapse in a Brokemon run had the same signature — `sick true`, health 0, hunger and thirst perfectly fine — the exact pattern already fixed for Brokedale. `treatFever` only knew about Brokedale's night market and was only ever called from the stranded routine, so a bot ill at home just bled out, on days two to four, holding $60–100, with the cure on a shelf at the Mart for **$12** the whole time. Collapsing clears the fever, which is why it stayed invisible: the run continues and the lost day looks like bad luck. Collapses **1.8 → 1.2 per run**. **The cure existed and was affordable and reachable — this was the rig, not the game** |

| 80 | St. Jude's had a tap you could leave running | "Sit quietly for a while" paid **+14 morale, +5 health and +6 energy for twenty minutes, unlimited**. Energy decays 2.6/hr, so a sit netted +5.1 — about **+15 energy an hour, free and forever**, against the +75 over eight hours a hostel bed *charges* for. A pew was 1.6x a bed, refilled health faster than the hospital sells it, and paid morale at +42/hr — the meter the bin food and the panhandling are priced against, so it handed back the dignity that taking food out of a bin is supposed to cost. The donation was worse in kind: $5 for +10 morale with **no time cost at all**, ten clicks for a full bar. **Neither harness has ever opened this door** — the walking rig has no reason to visit and no test named it — which is how an unlimited three-meter restore sat in the game unmeasured. Sitting down is still never refused, because shelter is the character of the place; what is once a day is the lift. Seven tests now cover it, four of which fail if the limit is removed |

| 81 | Every other door, checked the same way | Reading 2,700 lines of `venues.ts` would not reliably find the next one, so the audit is behavioural: take every option in both towns ten times over — four times of day, one and two menus deep, **339 trials** — and compare against simply letting the same game-minutes pass. Result after fixing the church: **zero** repeatable options that move a meter in zero minutes, and only four repeatable-and-free ones, all intentional and all bounded — the free shower (hygiene caps), tap water (thirst caps), the concourse (ten nights is 55 hours, rate −0.02), and the church itself, now paying **25 points across ten sits at a negative rate**. Kept as `venue-pumps.test.ts`; restoring either original bug fails it with the venue, the option and the number |
